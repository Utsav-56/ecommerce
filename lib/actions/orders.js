...
export async function placeOrderAction({ total, addressId, paymentMethod, couponCode, couponDiscount, cartItems }) {
  try {
    ...
    if (paymentMethod === 'COD') {
      ...
      redirectUrl = `/checkout/payment/esewa?orderId=${result.order.id}&amount=${total}`
      ...
      try {
        ...
        const khaltiResponse = await fetch('https://a.khalti.com/api/v2/epayment/initiate/', {
          method: 'POST',
          headers: {
            'Authorization': 'key 5c6a3e85cf5e4cf1922c2a05f5e4cf19',
      ...
      // Fallback
      if (!redirectUrl) {
        redirectUrl = `/checkout/payment?orderId=${result.order.id}&paymentId=${result.payment.id}&method=KHALTI`
      }
    } else {
      // Stripe / PayPal
      redirectUrl = `/checkout/payment?orderId=${result.order.id}&paymentId=${result.payment.id}&method=${paymentMethod}`
    }
