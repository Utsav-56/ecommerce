...
export async function placeOrderAction({ total, addressId, paymentMethod, couponCode, couponDiscount, cartItems }) {
  try {
    ...
    if (paymentMethod === 'COD') {
      ...
      if (!product_code || !secret) throw new Error('eSewa credentials missing in .env');
      ...
      try {
        ...
        if (!khaltiSecret) throw new Error('Khalti credentials missing in .env');
        ...
        const khaltiResponse = await fetch('https://a.khalti.com/api/v2/epayment/initiate/', {
          ...
          body: JSON.stringify({
            ...
            purchase_order_name: (await prisma.orderItem.findMany({ where: { orderId: result.order.id }, include: { product: true } })).map(i => i.product.name).join(', ').substring(0, 50),
            customer_info: {
    ...
    if (order.paymentMethod === 'ESEWA') {
      ...
      const khaltiResponse = await fetch('https://a.khalti.com/api/v2/epayment/initiate/', {
        ...
        body: JSON.stringify({
          ...
          // Fetch order items to get real product names
          purchase_order_name: (await prisma.orderItem.findMany({ where: { orderId: order.id }, include: { product: true } })).map(i => i.product.name).join(', ').substring(0, 50),
          customer_info: {
            name: dbUser?.name || 'Customer',
            email: dbUser?.email || 'customer@example.com',
            phone: '9800000000'
